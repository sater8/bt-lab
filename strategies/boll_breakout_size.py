...
class Strategy(bt.Strategy):
    params = dict(
        ...
        max_alloc_pct=None,   # se usa para capear el notional de la orden
        onramp_max=None,
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if cond_squeeze and cond_breakout and cond_vol and cond_trend:
                ...
                # Tamaño "teórico" por riesgo
                raw_size = risk_amount / risk_per_unit
                size = raw_size

                # 🔒 Ajuste para no pasar el cap de notional del middleware
                if self.p.max_alloc_pct is not None:
                    max_notional = equity * float(self.p.max_alloc_pct)
                    notional = size * close

                    if notional > max_notional:
                        # reducimos size al máximo permitido por el cap
                        size = max_notional / close
                ...
                self.trade_log.append({
                    ...
                    "equity": equity,
                    "max_notional": float(equity * self.p.max_alloc_pct) if self.p.max_alloc_pct is not None else None,
                })
                return
